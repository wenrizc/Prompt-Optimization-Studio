...
负责将评估报告、优化结果等制品持久化到磁盘, 并在数据库中维护索引记录。
"""
...
from backend.core.constants import ARTIFACT_OWNER_TYPES
...
def write_owner_artifact(
    ...
    Args:
        db: 数据库会话。
        owner_type: 所属者类型, 如 evaluation 或 optimization_run。
        owner_id: 所属者主键。
        artifact_type: 制品类型, 如 report 或 predictions。
        file_name: 文件名。
        payload: 要序列化为 JSON 的内容。

    Returns:
        创建或更新后的 Artifact 数据库记录。
    """
...
def write_manifest(db: Session, owner_type: str, owner_id: int) -> None:
    """根据数据库中的制品记录重新生成并写入 manifest 文件。"""
    ensure_valid_owner_type(owner_type)
    settings = get_settings()
    owner_dir = settings.artifacts_dir / _owner_dir(owner_type, owner_id)
    owner_dir.mkdir(parents=True, exist_ok=True)

    ...
    if (submitter === "validate") {
      try {
        const result = await api.post("/api/v1/prompts/validate", {
          ...
          output_schema_json: readJson(
            String(form.get("output_schema_json") ?? ""),
            selectedProject?.output_schema_json ?? { type: "object" },
          ),
        });
        ...
        <Panel title={t("prompts.createTitle")} description={t("prompts.createDescription")}>
          <form onSubmit={handleSubmit} className="grid gap-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <select name="status" defaultValue="active" className="rounded-2xl border border-stone-300 bg-white px-4 py-3">
                <option value="draft">{t("prompts.statusDraft")}</option>
                <option value="active">{t("prompts.statusActive")}</option>
                <option value="archived">{t("prompts.statusArchived")}</option>
              </select>
            </div>
            <textarea
              name="system_prompt"
            ...
            {filteredPrompts.map((prompt) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <StatusChip value={prompt.status} />
                </div>
              </button>
            ))}
          </div>
